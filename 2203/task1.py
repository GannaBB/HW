# Да се преработи следната задача, така че да използва речник.
...
while x < 1000:
    ...
    if B!=1 or (A==0 and B==0):
        ...
        if C==8: third_word='eigth'
        if C==9: third_word='nine'
    ...
    if A!=0:
        if B==0 and C==0:
            ...
            output = first_word + ' and ' + second_word + ' ' + third_word
    
    else: 
        output = first_word + ' ' + second_word + ' ' + third_word
...
try:
    n = 1000
    while n > 999:
        n = int(input('Enter your number in range 0..999: '))
   
        if n > 999:
            print('Too big value. Try again')
